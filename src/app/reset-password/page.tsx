...
export default function ForgotPassword() {
    ...
    function send() {
        ...
        api.post('/reset-password', {
            ...
            console.log(err)
            toast.error('Não foi possivel recuperar sua senha. Tente novamente mais tarde.');
        }).finally(() => {
            setLoading(false)
        });
        ...
        <main className="flex flex-1 w-screen h-screen items-center justify-center bg-gray-100 ">
            <div className="max-w-xl py-6 px-8 mt-20 bg-white rounded shadow-xl">
                ...
                <form action="">
                    ...
                    <a href="#!" type='button' onClick={() => !loading && send()} className="flex cursor-pointer py-2 px-4 items-center justify-center mt-6 bg-blue-500 hover:bg-blue-400 text-white font-bold w-full text-center rounded">
                        {!loading ? 'Redefinir senha' : <ThreeDots
                            visible={true}
                            height="30"
                            width="30"
                            color="white"
                    ...
                    <Link href="/login" className="text-base text-center w-full text-gray-800 mt-2 inline-block hover:text-indigo-600">Voltar</Link>
                </form>
            </div>
        </main>
    )
