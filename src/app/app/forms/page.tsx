...
import { PlusCircleIcon, EyeIcon, CircleStackIcon, CheckIcon } from "@heroicons/react/24/outline";
                        ...
                        forms.map((form) => (
                            <Card key={form.id}>
                                <h6 className="text-xl flex flex-row justify-between gap-2 font-semibold leading-4 tracking-tight text-gray-900 dark:text-white truncate">
                                    ...
                                    {form.published && <Badge icon={CheckIcon} className="pr-2 rounded-full" color="info">{form.submissions} </Badge>}
                                </h6>
                                <Link href={`/app/table/${form.tableId}`} className="font-normal bg-blue-50 self-start p-1 rounded-md leading-3 text-blue-700 dark:text-blue-200 truncate flex flex-row gap-1 items-center">
                                    <CircleStackIcon width={20} height={20} color="blue-700" /> {form.table?.title}
                                </Link>
                                ...
                                {
                                    !form.published &&
                                    <Button onClick={() => publish(form.id)} color="info" pill size="sm">
                                        Publicar
                                    </Button>
                                }
                                {
                                    form.published &&
                                    <Button onClick={() => viewSubmissions(form.id)} color="green" pill size="sm">
                                        Ver submissões
                                    </Button>
                                }
