...
import { HiOutlineHome, HiOutlinePlusSm, HiOutlineAdjustments, HiOutlineInbox, HiOutlineLogout, HiOutlineTable, HiOutlineUser, HiOutlineViewGrid, HiOutlineDatabase, HiOutlineUsers } from "react-icons/hi";
            ...
            <Sidebar aria-label="Menu Lateral" className="h-screen">
                ...
                <Sidebar.Items>
                    <Sidebar.ItemGroup>
                        ...
                        <Sidebar.Collapse open label="Tabelas" title="Tabelas" icon={HiOutlineDatabase}>
                            <Sidebar.Item href="#" icon={HiOutlinePlusSm} className="text-sm" onClick={handleCreateTable}>
                                Criar Tabela
                            </Sidebar.Item>
                            {
                                tables.map((table: TableType, index: number) => (
                                    <Sidebar.Item className={(tableId ? tableId == table.id && "bg-gray-200" : "")} key={index} href={`/app/table/${table.id}`} icon={HiOutlineTable}>
                                        {table.title}
                                    </Sidebar.Item>
                                ))
                            }
                        ...
                        <Sidebar.Item href="/app/notifications" icon={HiOutlineInbox} label="0">
                            Notificações
                        </Sidebar.Item>
                    ...
                    <Sidebar.ItemGroup>
                        ...
                        <Sidebar.Item icon={HiOutlineLogout} onClick={logout} className="cursor-pointer">
                            Sair
                        </Sidebar.Item>
                    </Sidebar.ItemGroup>
                </Sidebar.Items>
