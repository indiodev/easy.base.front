...
export function CreateFormBtn({ tableId, tableName }: CreateFormBtnProps) {
    ...
    const { handleSubmit, register, getValues, formState, setValue } = useForm<formSchemaType>({
    ...
    function onSubmit(values: formSchemaType) {
        try {
            api.post('/forms', {
                ...
                if (data.data.erro)
                    toast.error('Erro ao tentar criar formulario!');
                else {
                ...
                <Modal.Body>
                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                        <div>
                            ...
                            <TextInput {...register("title")} id="title" type="text" placeholder={"Formulario de " + tableName} required />
                        </div>
                        <div>
                            <div className="mb-2 block">
                                <Label htmlFor="description" value="Descrição" />
                            </div>
                            <Textarea {...register("description")} id="description" placeholder="Escreva uma descrição..." required rows={4} />
                        </div>
                        <div className="flex justify-between gap-4">
                            <Button color="blue" type="submit" disabled={formState.isSubmitting}>
                                Confirmar
                                {formState.isSubmitting && <div className="loader loader-white ml-2"></div>}
