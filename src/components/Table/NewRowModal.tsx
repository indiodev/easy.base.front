...
import { ColumnType, DATA_TYPES, TableType } from "@/contexts/types";
...
import { DateCustomInput } from "./DateCell";
...
export function NewRowModal({ open, setOpen, fields, onReload }: NewRowModalType) {
  ...
  const { control, register, handleSubmit, setValue, getValues, watch, reset } = useForm<newRowSchema>({
  ...
  function onSubmit(data: newRowSchema) {
    ...
    api.post(`/tables/${id}/row`, {
      ...
      if (data.data.erro)
        toast.error('Erro ao tentar criar registro!');
      else {
      ...
      console.log(err)
      toast.error('Não foi possivel criar o registro. Tente novamente mais tarde.');
    }).finally(() => {
      setLoading(false)
      onCloseModal()
    });
  ...
  function GeneratedFieldInput({ field }: DynamicFieldType) {
    ...
    if (field.type == 'selectcell' || field.identifier == 'selectcell') {
      ...
      return <Select id="countries" required {...register(`data.${field.slug}`)}>
    ...
    if (field.type == 'datecell')
      return <TextInput type="datetime-local" placeholder={`Por favor, informe o: ${field.title}`} required {...register(`data.${field.slug}`)} />


    if (field.type == 'editablecell')
      return <TextInput placeholder={`Por favor, informe o: ${field.title}`} required {...register(`data.${field.slug}`)} />
  ...
  return (
    <>
      <Modal show={open} size="xl" onClose={onCloseModal} popup>
        ...
        <Modal.Body>
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">Insira um registro</h3>
          <div className="space-y-2">
            <form name="form" onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
