...
import { DATA_TYPES, TableType } from "@/contexts/types";
...
export function CreateTableModal({ open, setOpen }: TableModalType) {
  ...
  const { control, register, handleSubmit, setValue, getValues, watch, reset } = useForm<CreateTableSchema>({
  ...
  function handleCreate(data: CreateTableSchema) {
    ...
    api.post('/tables', {
      ...
      if (data.data.erro)
        toast.error('Erro ao tentar criar tabela!');
      else {
      ...
      console.log(err)
      toast.error('Não foi possivel criar a tabela. Tente novamente mais tarde.');
    }).finally(() => {
      setLoading(false)
      onCloseModal()
    });
  ...
  return (
    <>
      <Modal show={open} size="xl" onClose={onCloseModal} popup>
        ...
        <Modal.Body>
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">Crie uma tabela</h3>
          <div className="space-y-2">
            <form name="form" onSubmit={handleSubmit(handleCreate)} className="flex flex-col gap-4">
              <div>
                <Label htmlFor="title" value="Título" />
                <TextInput placeholder="Insira o nome da tabela" required {...register('title')} />
              </div>


              <div className="flex gap-2 flex-row justify-between">
                <Label htmlFor="title" value="Colunas" />
                <Button color="light" size="xs" className="self-start" onClick={addColumn}>Adicionar coluna</Button>
              </div>
                ...
                fields.map((column, ind) => (
                  <div key={column.id} className="gap-2 flex flex-col">
                    <div className="flex flex-row gap-2">
                      <TextInput className="w-full" placeholder="Titulo" {...register(`columns.${ind}.title`)} />
                      <Select id="columnType" className="w-full" {...register(`columns.${ind}.type`)}>
                        {
                      ...
                      {
                        <Button color="failure" onClick={() => rmColumn(ind)}><HiTrash className="h-5 w-5" /></Button>
                      }
                    </div>

                    {
                      ...
                      <div className="flex flex-col gap-2 p-2 border-gray-400 border rounded-lg shadow-sm">
                        ...
                        <div className="flex gap-2 flex-row justify-between">
                          ...
                          <Button color="light" size="xs" className="self-start" onClick={() => addOption(ind)}>Adicionar Opcao</Button>
                        </div>

                        {
                          column.config?.options?.map((option: any, opc: number) => (
                            <div key={opc} className="flex flex-row gap-2">
                              <TextInput className="w-full" placeholder="Nome" {...register(`columns.${ind}.config.options.${opc}.name`)} />
                              <Button color="failure" onClick={() => rmOption(ind, opc)}><HiTrash className="h-5 w-5" /></Button>
                            </div>
                          ))
                        }
